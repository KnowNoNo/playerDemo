 ...
 * @param[in] encodetype 对采集数据进行编码设置，0：pcm，1：g711a，2：g711u。pcm回调裸数据，g711a和g711u回调带私有(新)封装头的数据
 * @return BOOL，成功返回TURE，失败返回FALSE
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
...
typedef enum _IVS_TYPE
{
	...
	IVSINFOTYPE_DHOP_SMART			= 22,	//DHOP开放平台智能帧(0x14)
}IVS_TYPE;
