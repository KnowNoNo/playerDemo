...
void CBitmapSlider::OnMouseMove(UINT nFlags, CPoint point) 
{
	...
	if( !m_bLButtonDown)
	{
		...
		//{
		//	strTips.Format(L"%d%%", this->GetPos());
	...
	if( Pos2Pixel(m_nPos) != nPixel ) {
		...
		if(m_ctlTips.m_hWnd != NULL)
		{
			strTips.Format(L"%d%%", this->GetPos()*100/(m_nMax-m_nMin));
			m_ctlTips.UpdateTipText(strTips, this);
		}
...
BOOL CBitmapSlider::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message)
{
...
void CBitmapSlider::OnMouseLeave()
{
	// TODO: 在此添加消息处理程序代码和/或调用默认值
